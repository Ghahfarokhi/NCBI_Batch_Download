...
Error_Handler:
    If Err.Number <> 0 Then
        ...
        Sheets("Accession").Range("Comments").Offset(Batch, 0).Style = "Bad"
        Err.Number = 0
    Else
        Annotator = True
    End If
...
Function Seq_Extractor(Batch As Long, GenBank_File_Path As String, File_Type As String) As String
    '********************************************
    '*************** ERROR POLICY ***************
    '********************************************
    Application.DisplayAlerts = False
    ...
    If File_Type = ".gb" Or File_Type = ".gp" Then
        ORIGIN_Found = False
...
Next_Loop:
    ...
    ElseIf File_Type = ".fasta" Then
        ...
        While Not EOF(1)
            ...
            DataLine = Replace(Replace(Replace(DataLine, Chr(10), "§"), Chr(13), "§"), vbNewLine, "§")
